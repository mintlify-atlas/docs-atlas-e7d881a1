---
title: "Install Sanctifier CLI and Dependencies"
...
Sanctifier is written in Rust and requires **Rust 1.75 or later**. Install the Rust toolchain via [rustup](https://rustup.rs/):
...
<Tabs>
  <Tab title="Cargo (from source)">
    ...
    You should see the top-level help output listing all available subcommands (`analyze`, `verify`, `prove`, `baseline`, `diff`, `attest`, `cve`, and more).
...
For teams using `sanctifier deploy` or the runtime-guard deployment scripts, a handful of environment variables control network and deployment behavior. Copy the provided template to a local file that is excluded from version control:
